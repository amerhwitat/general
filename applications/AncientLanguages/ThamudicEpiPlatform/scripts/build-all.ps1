...
& (Join-Path $Nlp 'scripts\build-all-languages.ps1')
$python = Join-Path $Nlp '.venv\Scripts\python.exe'
& $python -m pytest (Join-Path $Nlp 'server\tests') -q
if (Get-Command npm -ErrorAction SilentlyContinue) { Push-Location (Join-Path $Nlp 'web'); npm install; npm run build; Pop-Location }
