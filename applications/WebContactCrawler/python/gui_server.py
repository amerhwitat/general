...
def _run_job(job_id, cfg):
    ...
    try:
        events = Path(cfg["events"]); events.parent.mkdir(parents=True, exist_ok=True)
        crawler = Crawler([cfg["url"]], cfg["keywords"], cfg["max_pages"], cfg["max_depth"], cfg["delay"], cfg["timeout"], cfg["max_bytes"], event_file=events, stop_event=job["stop_event"])
        rows = crawler.run(); output = Path(cfg["output"]); output.parent.mkdir(parents=True, exist_ok=True); export(rows, output)
        with LOCK:
            job["state"] = "cancelled" if job["stop_event"].is_set() else "done"
...
class Handler(BaseHTTPRequestHandler):
    server_version = "WebContactCrawlerGUI/1.1"
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path == "/api/scan":
                ...
                job_id = uuid.uuid4().hex[:12]; events = output.parent / (output.stem + ".events.jsonl")
                cfg = {"url": url, "output": str(output), "events": str(events), "keywords": [str(x) for x in data.get("keywords", []) if str(x).strip()], "max_pages": max(1, min(int(data.get("max_pages", 100)), 100000)), "max_depth": max(0, min(int(data.get("max_depth", 3)), 32)), "delay": max(0.0, float(data.get("delay", 0.5))), "timeout": max(1.0, min(float(data.get("timeout", 10)), 120)), "max_bytes": max(1024, min(int(data.get("max_bytes", 2000000)), 50000000))}
                job = {"id": job_id, "state": "queued", "created": time.time(), "output": str(output), "events": str(events), "stop_event": threading.Event()}
